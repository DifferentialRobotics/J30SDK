...
class UDPClientProtocol(asyncio.DatagramProtocol):
    ...
    def connection_made(self, transport):
        ...
        print("客户端已连接到服务器")
        send_command_request("waypoints")
...
def create_waypoints_request(x=6.0, y=0.0, z=1.0):
    """创建发送waypoints请求消息"""
    request = {
        "version": "1.1.0",
...
def send_command_request(command_type):
    ...
    try:
        if command_type == "waypoints":
            request_data = create_waypoints_request(x=6.0, y=0.0, z=1.0)
            print(f"发送waypoints请求")
        else:
